...
function exportarCSV() {
  ...
  link.setAttribute("download", `reporte_${reporteActual}_${new Date().toISOString().split('T')[0]}.csv`);
  link.style.visibility = 'hidden';
  document.body.appendChild(link);
  link.click();
  document.body.removeChild(link);
}">
                    Última conexión: ${e.ultimaConexion.toLocaleString('es-MX')}
                  </p>
                </div>
              </div>
              <div class="text-right">
                <p class="font-bold">${statusText}</p>
                ${e.estado !== 'online' ? `<p class="text-sm text-gray-600">Hace ${e.minutosDesconectado < 1440 ? Math.floor(e.minutosDesconectado) + ' minutos' : e.diasDesconectado + ' días'}</p>` : ''}
              </div>
            </div>
          `;
        }).join('')}
  ...
  datosReporte = { estadoConexion, resumen: { online, warning, offline } };
}

async function reporteMantenimientoPredictivo(contenedor) {
  const { ventas, maquinas } = await obtenerDatosBase();
  
  // Calcular litros vendidos por máquina
  const LITROS_MANTENIMIENTO = 10000; // Ejemplo: mantenimiento cada 10,000 litros
  
  const mantenimientoInfo = maquinas.map(m => {
    const ventasMaquina = ventas.filter(v => v.serial === m.serial);
    const litrosVendidos = ventasMaquina.reduce((sum, v) => sum + parseFloat(v.litros || 0), 0);
    const porcentajeUso = (litrosVendidos % LITROS_MANTENIMIENTO) / LITROS_MANTENIMIENTO * 100;
    const litrosFaltantes = LITROS_MANTENIMIENTO - (litrosVendidos % LITROS_MANTENIMIENTO);
    
    // Calcular días estimados basándose en promedio
    const diasAnalisis = 30;
    const ventasRecientes = ventasMaquina.filter(v => 
      (new Date() - new Date(v.created_at)) / (1000 * 60 * 60 * 24) <= diasAnalisis
    );
    const litrosPorDia = ventasRecientes.reduce((sum, v) => sum + parseFloat(v.litros || 0), 0) / diasAnalisis;
    const diasEstimados = litrosPorDia > 0 ? Math.ceil(litrosFaltantes / litrosPorDia) : 999;
    ...
    return {
      ...
      litrosFaltantes,
      diasEstimados,
      estado: porcentajeUso > 90 ? 'urgente' : porcentajeUso > 70 ? 'proximo' : 'ok'
    };
  });
  
  mantenimientoInfo.sort((a, b) => b.porcentajeUso - a.porcentajeUso);
  ...
  contenedor.innerHTML = `
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8">
      <div class="bg-red-50 dark:bg-red-900 p-4 rounded">
        <p class="text-sm text-gray-600 dark:text-gray-300">Mantenimiento Urgente</p>
        <p class="text-3xl font-bold text-red-600">${mantenimientoInfo.filter(m => m.estado === 'urgente').length}</p>
      </div>
      <div class="bg-yellow-50 dark:bg-yellow-900 p-4 rounded">
        <p class="text-sm text-gray-600 dark:text-gray-300">Próximo Mantenimiento</p>
        <p class="text-3xl font-bold text-yellow-600">${mantenimientoInfo.filter(m => m.estado === 'proximo').length}</p>
      </div>
      <div class="bg-gray-50 dark:bg-gray-800 p-4 rounded">
        <p class="text-sm text-gray-600 dark:text-gray-300">Intervalo de Mantenimiento</p>
        <p class="text-3xl font-bold">${LITROS_MANTENIMIENTO.toLocaleString()} L</p>
      </div>
    </div>
    
    <!-- Lista de máquinas -->
    <div class="space-y-4">
      ${mantenimientoInfo.map(m => {
        const barColor = m.estado === 'urgente' ? 'bg-red-500' : 
                        m.estado === 'proximo' ? 'bg-yellow-500' : 'bg-green-500';
        
        return `
          <div class="bg-white dark:bg-gray-800 p-4 rounded-lg">
            <div class="flex justify-between items-start mb-2">
              <h3 class="font-bold">${m.nombre}</h3>
              <span class="text-sm ${m.estado === 'urgente' ? 'text-red-600' : m.estado === 'proximo' ? 'text-yellow-600' : 'text-green-600'}">
                ${m.diasEstimados < 999 ? `${m.diasEstimados} días` : 'Sin datos'}
              </span>
            </div>
            <div class="w-full bg-gray-200 dark:bg-gray-700 rounded-full h-4 mb-2">
              <div class="${barColor} h-4 rounded-full" style="width: ${m.porcentajeUso}%"></div>
            </div>
            <p class="text-sm text-gray-600 dark:text-gray-400
