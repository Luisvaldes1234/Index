...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  // 3.3. Cargar máquinas en <select>
  const { data: maquinas, error: errM } = await supabase
    .from('maquinas')
    .select('id, name')
    .eq('usuario_id', user.id)
  if (errM) console.error('Error máquinas:', errM.message)
  else {
  ...
  document.getElementById('btnToggleDark').addEventListener('click', () => document.body.classList.toggle('dark'))
...
async function actualizarResumen() {
  ...
  // Mostrar/ocultar mensaje “sin ventas”
...
function actualizarGraficaVentasDiarias(ventas) {
  ...
  const mapa = {}
  ventas.forEach(v => {
    const d = v.fecha.slice(0,10)     // 'YYYY-MM-DD'
...
function cerrarSesion(){
  supabase.auth.signOut().then(_=>window.location.href='login.html')
